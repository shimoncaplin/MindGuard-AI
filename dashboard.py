...
def generate_client_share_pdf(current_df, current_analysis, workspace_name):
    ...
    if os.path.exists(logo_path):
        try:
            logo = Image(logo_path, width=4.1 * cm, height=1.55 * cm)
            story.append(logo)
    ...
    score_data = [
        ...
        ["Reliability Score", f"{reliability}%"],
        ["Viability", f"{readiness_score}/100"],
        ["Deployment Verdict", str(readiness_label)],
        ["Critical Issues", str(current_analysis.get("bad_count", 0))],
        ["Weak Responses", str(current_analysis.get("weak_count", 0))],
    ]

    score_table = Table(score_data, colWidths=[7 * cm, 9 * cm])
    ...
    verdict_data = [
        ["Verdict", str(verdict_title)],
        ["Summary", str(verdict_summary)],
        ["Recommended Action", str(verdict_action)],
    ]
    verdict_table = Table(verdict_data, colWidths=[4.2 * cm, 11.8 * cm])
    ...
    if current_df is not None and not current_df.empty:
        timeline_rows = [["Timestamp", "Event", "Score", "Status"]]
        ...
        timeline_table = Table(timeline_rows, colWidths=[4.6 * cm, 6.4 * cm, 2 * cm, 3 * cm])
    ...
    if current_df is not None and not current_df.empty:
        incident_rows = [["Score", "Status", "Prompt", "Response"]]
        for _, row in current_df.head(5).iterrows():
            incident_rows.append([
                str(row.get("score", "")),
                str(row.get("status", "")),
                str(row.get("prompt", ""))[:65],
                str(row.get("response", ""))[:75],
            ])

        incident_table = Table(incident_rows, colWidths=[1.5 * cm, 2.2 * cm, 6.1 * cm, 6.2 * cm])
