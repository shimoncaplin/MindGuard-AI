...
def generate_client_share_pdf(current_df, current_analysis, workspace_name):
    ...
    def PB(value):
        return Paragraph(escape(str(value)), body_bold_style)
    ...
    story.append(Paragraph("AI Quality Assessment Report", title_style))
    story.append(Paragraph("Generated by MindGuard AI", subtitle_style))

    meta_data = [
        [PB("Workspace"), P(workspace_name)],
        [PB("Generated Timestamp"), P(generated_at)],
        [PB("Report Type"), P("Client Share / Board-Ready Quality Report")],
    ]
    meta_table = Table(meta_data, colWidths=[4.5 * cm, 11.8 * cm], hAlign="LEFT")
    ...
    score_data = [
        [PB("Metric"), PB("Value")],
        [P("AI Health"), P(f"{current_analysis.get('health', 0)}/100")],
        [P("Quality Score"), P(current_analysis.get("avg_score", 0))],
        [P("Reliability Score"), P(f"{reliability}%")],
        [P("Viability"), P(f"{readiness_score}/100")],
        [P("Deployment Verdict"), P(readiness_label)],
        [P("Critical Issues"), P(current_analysis.get("bad_count", 0))],
        [P("Weak Responses"), P(current_analysis.get("weak_count", 0))],
    ]

    score_table = Table(score_data, colWidths=[6.2 * cm, 10.1 * cm], hAlign="LEFT")
    score_table.setStyle(TableStyle([
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#0B55D8")),
    ...
    story.append(Spacer(1, 8))
    story.append(Paragraph("Latest Timeline", h_style))

    if current_df is not None and not current_df.empty:
        timeline_rows = [[PB("Timestamp"), PB("Event"), PB("Score"), PB("Status")]]
        for _, row in current_df.head(7).iterrows():
            status = str(row.get("status", "UNKNOWN")).upper()
            if status == "GOOD":
                event = "Quality test completed"
            elif status == "WEAK":
            ...
            timeline_rows.append([
                P(str(row.get("timestamp", ""))[:22], small_style),
                P(event, small_style),
                P(str(row.get("score", "")), small_style),
                P(status, small_style),
            ])

        timeline_table = Table(timeline_rows, colWidths=[4.2 * cm, 6.5 * cm, 2 * cm, 3.6 * cm], hAlign="LEFT", repeatRows=1)
        ...
        story.append(Paragraph("No timeline activity available.", body_style))

    story.append(Spacer(1, 8))
    story.append(Paragraph("Latest Client-Visible Incidents", h_style))

    if current_df is not None and not current_df.empty:
        incident_rows = [[PB("Score"), PB("Status"), PB("Prompt"), PB("Response")]]
        for _, row in current_df.head(5).iterrows():
            incident_rows.append([
                P(str(row.get("score", "")), small_style),
                P(str(row.get("status", "")), small_style),
                P(str(row.get("prompt", "")), small_style),
                P(str(row.get("response", "")), small_style),
            ])

        incident_table = Table(incident_rows, colWidths=[1.4 * cm, 2.2 * cm, 6.2 * cm, 6.5 * cm], hAlign="LEFT", repeatRows=1)
        ...
        story.append(Paragraph("No incident data available.", body_style))

    story.append(Spacer(1, 10))
    story.append(Paragraph("MindGuard AI — monitor AI before it fails.", small_style))
