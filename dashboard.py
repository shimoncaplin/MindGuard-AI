...
def load_benchmark_csv(uploaded_file):
    ...
    if missing:
        raise ValueError("Benchmark CSV is missing required columns: " + ", ".join(missing))
    return df
...
def create_benchmark_summary(results_df):
    ...
    if average_score >= 85 and average_risk < 40 and failed_tests == 0:
        readiness = "PRODUCTION READY"
    elif average_score >= 70 and failed_tests == 0:
        readiness = "NEEDS MORE TESTING"
    elif average_score >= 65 and failed_tests <= max(1, int(total_tests * 0.2)):
        readiness = "NEEDS MORE TESTING"
    else:
        readiness = "NOT READY"
...
def generate_memory_training_plan(analysis=None, failure_summary_df=None, failed_prompt_details_df=None):
    recommendations = []
    memory_score = int(analysis.get("memory", 0)) if analysis else 0
    context_score = int(analysis.get("context", 0)) if analysis else 0
    hallucination_risk = int(analysis.get("hallucination_risk", 0)) if analysis else 0
    bad_count = int(analysis.get("bad_count", 0)) if analysis else 0
    ...
    return pd.DataFrame(recommendations).drop_duplicates(subset=["Training Area", "Recommended Rule"], keep="first")
...
def generate_memory_rules_text(training_plan_df):
    ...
    lines = ["MindGuard AI Recommended Memory Rules", ""]
...
def _root_short_fact_correct(prompt, response):
...
def classify_failure_reason(row):
    ...
    if _root_short_fact_correct(prompt, response):
        return {
    ...
    reasons, fixes = [], []
    prompt_words = set(w.lower().strip(".,?!:;()[]{}\"'") for w in prompt.split() if len(w) > 4)
...
def create_root_cause_report(df):
    ...
    if not target_df.empty:
        target_df = target_df[~target_df.apply(lambda row: _root_short_fact_correct(row.get("prompt", ""), row.get("response", "")), axis=1)]
    ...
    return result_df.sort_values(["Score", "ID"], ascending=[True, False])
...
def summarize_root_causes(root_cause_df):
    ...
    if "Status" in root_cause_df.columns and ("NO DATA" in root_cause_df["Status"].astype(str).values or "HEALTHY" in root_cause_df["Status"].astype(str).values):
        return {
            "critical_count": 0,
            "weak_count": 0,
    ...
    readiness_impact = "BLOCKS DEPLOYMENT" if critical_count > 0 else ("NEEDS REVIEW" if weak_count > 0 else "LOW IMPACT")
    return {
